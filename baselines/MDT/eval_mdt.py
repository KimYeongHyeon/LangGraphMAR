#!/usr/bin/env python
"""
MDT test set evaluation.

Uses the same paper-metric computation as eval_nmar.py for fair comparison.

Usage:
    cd <repo_root>/code
    python ../baselines/MDT/eval_mdt.py --anatomy body [--iterations 2]
    python ../baselines/MDT/eval_mdt.py --anatomy head
"""
...
# Mirror eval_nmar.py: put the project `code/` on sys.path and chdir to it
CODE_DIR = Path(__file__).parent.parent.parent / "code"
sys.path.insert(0, str(CODE_DIR))
os.chdir(CODE_DIR)

from phasepack import phasecong  # noqa: E402
from utils.dataset import IndexManager, load_data_list  # noqa: E402
from utils.projection import filtering, bp  # noqa: E402
from utils.ct import transform_image_unit_mm_to_HU  # noqa: E402

# Local import (MDT module)
sys.path.insert(0, str(Path(__file__).parent))
from mdt import mdt, setup_ct_params  # noqa: E402


# ---- Paper-metric functions (copied verbatim from eval_nmar.py for identity) ----
...
def main():
    ...
    parser.add_argument("--iterations", type=int, default=2,
                        help="Number of MDT passes (1=LI, 2=default MDT, 3+=more refinement).")
    parser.add_argument("--output_dir", type=str, default=None,
                        help="If set, save each sample's MDT HU image here.")
    parser.add_argument("--limit", type=int, default=None,
                        help="Process only first N samples (for quick test).")
    ...
    if args.limit:
        ma_sino_paths = ma_sino_paths[: args.limit]
        gt_sino_paths = gt_sino_paths[: args.limit]
        mask_img_paths = mask_img_paths[: args.limit]
    ...
    print(
        f"MDT evaluation: {args.anatomy}, {len(ma_sino_paths)} test samples, "
        f"iterations={args.iterations}"
    )

    all_ssim, all_fsim, all_rmse, times = [], [], [], []

    for i in tqdm(range(len(ma_sino_paths)), desc=f"MDT({args.anatomy})"):
        ...
        # GT: reconstruct from GT sinogram (same as sweep / eval_nmar)
        gt_sino = np.fromfile(str(gt_sino_paths[i]), dtype=np.float32).reshape(1000, 900)
        ct_fbp = ct_param.copy()
        ct_fbp["filter"] = "ram-lak"
        gt_mm = np.maximum(
            bp(filtering(gt_sino, ct_fbp), ct_fbp).astype(np.float32), 0
        )
        gt_HU = transform_image_unit_mm_to_HU(gt_mm)

        # Run MDT
        t0 = time.perf_counter()
        mdt_mm = mdt(sino, mask_bin, args.anatomy, ct_param, iterations=args.iterations)
        elapsed = (time.perf_counter() - t0) * 1000
        times.append(elapsed)

        mdt_HU = transform_image_unit_mm_to_HU(mdt_mm)

        metrics = compute_metrics(gt_HU, mdt_HU, mask_bin, args.anatomy)
        all_ssim.append(metrics["ssim"])
        all_fsim.append(metrics["fsim"])
        all_rmse.append(metrics["rmse"])

        if args.output_dir:
            mdt_HU.astype(np.float32).tofile(
                os.path.join(args.output_dir, f"mdt_{i:05d}.raw")
            )
    ...
    print(f"\n{'='*60}")
    print(f"MDT Results ({args.anatomy}, n={len(ma_sino_paths)}, iters={args.iterations})")
    print(f"{'='*60}")
    ...
    df = pd.DataFrame(
        {"ssim": all_ssim, "fsim": all_fsim, "rmse": all_rmse, "time_ms": times}
    )
    out_dir = CODE_DIR.parent / "logs"
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = str(out_dir / f"mdt_metrics_{args.anatomy}.csv")
